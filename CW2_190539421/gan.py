...
#---------Generated results for 100 epochs--------------------------------------
def show_result(num_epoch, show = True, save = True, path = 'result.png'):
    ...
    im = sess.run(G_z, {z: z_, drop_out: 0.0})
    ...
    for i in range(5):
      for j in range(5):
        ax[i, j].imshow(np.reshape(im[k], (28, 28)), cmap='gray')
        k = k+1
    l = 'Epoch {0}'.format(num_epoch)
    fig.text(0.5, 0.04, l, ha='center')
    plt.savefig(path)
    if show and (num_epoch == 10 or num_epoch == 20 or num_epoch == 50 or num_epoch == 100):
      plt.show()
